...
module TwitterAds
  class AudienceIntelligence
    ...
    RESOURCE_CONVERSATIONS  = "/#{TwitterAds::API_VERSION}/" +
                           'accounts/%{account_id}/audience_intelligence/conversations'.freeze # @api private
    RESOURCE_DEMOGRAPHICS   = "/#{TwitterAds::API_VERSION}/" +
                           'accounts/%{account_id}/audience_intelligence/demographics'.freeze # @api private
    ...
    def conversations()
      ...
      resource = RESOURCE_CONVERSATIONS % { account_id: self.account.id }
      request = Request.new(self.account.client, :post, resource, body: params.to_json, headers: headers)
      Cursor.new(self.class, request, init_with: [self.account])
    end
    ...
    def demographics()
      ...
      resource = RESOURCE_DEMOGRAPHICS % { account_id: self.account.id }
      response = Request.new(self.account.client, :post, resource, body: params.to_json, headers: headers).perform
